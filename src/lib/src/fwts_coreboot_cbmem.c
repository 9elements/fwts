...
ssize_t memconsole_coreboot_read(char *buf, size_t pos, size_t count)
{
	uint32_t cursor = cbmem_console->cursor & CURSOR_MASK;
	uint32_t flags = cbmem_console->cursor & ~CURSOR_MASK;
	uint32_t size = cbmem_console_size;
...
int find_cbmem_entry(uint32_t id, uint64_t *addr, size_t *size)
{
        const uint8_t *table;
        size_t offset;
        int ret = -1;
